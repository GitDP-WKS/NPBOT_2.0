...
class TrainingRow:
    ...
    group: str
    weight: float
    address_type: str = "неопределенный"
...
def fit(rows: list[TrainingRow]) -> Pipeline:
    ...
    model.fit(
        ...
        classifier__sample_weight=np.array([row.weight for row in rows]),
    )
    return model
...
def predict_options(model: Pipeline, text: str, top_n: int = 3) -> list[dict[str, Any]]:
    ...
    return [
        {"res": str(classes[index]), "probability": float(probabilities[index])}
        for index in order
    ]
