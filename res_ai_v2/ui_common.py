...
FIELDS = {"Филиал": "branch", "РЭС": "res", "Населенный пункт": "locality", "Район": "district", "СНТ / поселок": "settlement", "Улица": "street"}


def configure() -> None:
    try:
        if st.secrets.get("DATABASE_URL"): os.environ["DATABASE_URL"] = str(st.secrets["DATABASE_URL"])
        if st.secrets.get("ADMIN_PASSWORD"): os.environ["ADMIN_PASSWORD"] = str(st.secrets["ADMIN_PASSWORD"])
...
def admin_login() -> bool:
    if st.session_state.get("is_admin"):
        if st.sidebar.button("Выйти из служебного режима", use_container_width=True): st.session_state["is_admin"] = False; st.rerun()
        return True
    with st.sidebar.expander("Служебный вход"):
        ...
        if st.button("Войти", use_container_width=True):
            ...
            if expected and password == expected: st.session_state["is_admin"] = True; st.rerun()
            else: st.error("Неверный пароль.")
    return False


def address_table(rows: list[dict[str, Any]]) -> pd.DataFrame:
    return pd.DataFrame([{title: row.get(field, row.get({"branch":"branch_name","res":"res_name"}.get(field, field), "")) for title, field in FIELDS.items()} for row in rows], columns=ORDER)


def display_options(rows: list[dict[str, Any]]) -> None:
    if rows: st.dataframe(address_table(rows), use_container_width=True, hide_index=True)
