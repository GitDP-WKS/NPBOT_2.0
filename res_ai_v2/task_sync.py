...
def sync_review_tasks(
    ...
    *,
    ...
    """Одной транзакцией создает, обновляет и закрывает задания агента."""
    initialize_database()
    now = utcnow()
    ...
    with get_engine().begin() as conn:
        ...
        for key, task in unique.items():
            ...
            if key in existing:
                updates.append({"b_id": existing[key], **{f"b_{name}": value for name, value in values.items()}})
            else:
        ...
        if close_stale:
            ...
            query = select(review_tasks.c.id, review_tasks.c.task_key).where(
                ...
                review_tasks.c.task_type.in_(types),
            )
            stale_ids = [
                int(row.id)
