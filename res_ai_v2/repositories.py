...
from .schema import addresses, address_mappings, audit_events, model_versions, query_rules, review_tasks, review_votes, source_files, source_rows, mapping_evidence, text_examples, review_decisions


def audit(actor: str, action: str, entity_type: str, entity_key: str, before: Any = None, after: Any = None) -> None:
    initialize_database()
    with get_engine().begin() as conn:
        conn.execute(insert(audit_events).values(actor=actor or "system", action=action, entity_type=entity_type, entity_key=entity_key, before_json=stable_json(before or {}), after_json=stable_json(after or {}), created_at=utcnow()))
...
def create_or_update_task(*, task_key: str, task_type: str, subject_type: str, subject_key: str, title: str, payload: dict[str, Any], priority: int) -> int:
    initialize_database(); now = utcnow(); encoded = stable_json(payload)
    with get_engine().begin() as conn:
        row = conn.execute(select(review_tasks.c.id).where(review_tasks.c.task_key == task_key)).first()
        values = dict(task_type=task_type, subject_type=subject_type, subject_key=subject_key, title=title, payload_json=encoded, priority=priority, status="open", updated_at=now)
        if row:
            conn.execute(update(review_tasks).where(review_tasks.c.id == row.id).values(**values)); return int(row.id)
        return int(conn.execute(insert(review_tasks).values(task_key=task_key, created_at=now, **values)).inserted_primary_key[0])


def close_stale_tasks(task_types: Iterable[str], keep_keys: set[str]) -> int:
    with get_engine().begin() as conn:
        rows = conn.execute(select(review_tasks.c.id, review_tasks.c.task_key).where(review_tasks.c.status == "open", review_tasks.c.task_type.in_(list(task_types)))).all()
        stale = [int(row.id) for row in rows if str(row.task_key) not in keep_keys]
        if stale:
            conn.execute(update(review_tasks).where(review_tasks.c.id.in_(stale)).values(status="cancelled", updated_at=utcnow()))
        return len(stale)


def list_review_tasks(reviewer: str, limit: int = 50) -> list[dict[str, Any]]:
    initialize_database(); reviewer_key = normalize_text(reviewer)
    with get_engine().connect() as conn:
        reviewed = set(conn.scalars(select(review_votes.c.task_id).where(review_votes.c.reviewer == reviewer_key))) if reviewer_key else set()
...
def increment_human_decisions() -> int:
    value = int(get_setting("human_decisions_since_training", "0")) + 1; set_setting("human_decisions_since_training", value); return value
...
def lookup_query_rule(raw_query: str) -> list[dict[str, Any]] | None:
    ...
    with get_engine().connect() as conn: row = conn.execute(select(query_rules.c.selection_json).where(query_rules.c.normalized_query == normalized)).first()
    return json.loads(str(row.selection_json) or "[]") if row else None


def delete_query_rule(raw_query: str) -> None:
    with get_engine().begin() as conn: conn.execute(delete(query_rules).where(query_rules.c.normalized_query == normalize_text(raw_query)))


def recent_audit(limit: int = 200) -> list[dict[str, Any]]:
    with get_engine().connect() as conn: return [dict(row._mapping) for row in conn.execute(select(audit_events).order_by(audit_events.c.id.desc()).limit(limit))]
...
def browse_knowledge(search_text: str = "", status: str = "", branch: str = "", res_name: str = "", limit: int = 500) -> list[dict[str, Any]]:
    ...
    if res_name: conditions.append(address_mappings.c.res_name == res_name)
    if search_text.strip():
        pattern = f"%{search_text.strip()}%"; conditions.append(or_(addresses.c.locality.ilike(pattern), addresses.c.district.ilike(pattern), addresses.c.settlement.ilike(pattern), addresses.c.street.ilike(pattern)))
    ...
    with get_engine().connect() as conn:
        for table in tables:
            ...
            if table is model_versions:
                for row in rows:
                    if row.get("model_blob") is not None: row["model_blob"] = base64.b64encode(bytes(row["model_blob"])).decode("ascii")
